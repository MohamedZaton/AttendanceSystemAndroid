...
public class LogExcusesAdapter extends  RecyclerView.Adapter<LogExcusesAdapter.ViewHolder> {
    private List<LogExcusesModel> ReqList;
    private Context context;

    public LogExcusesAdapter(List<LogExcusesModel> reqList, Context context) {
        ReqList = reqList;
    ...
    @NonNull
    @Override
    public LogExcusesAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_item_attendance, parent, false);

        LogExcusesAdapter.ViewHolder viewHolder = new LogExcusesAdapter.ViewHolder(view);
        return viewHolder;
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onBindViewHolder(@NonNull LogExcusesAdapter.ViewHolder holder, int position) {
        LogExcusesModel logExcusesModel = ReqList.get(position);


        holder.img_iconAtt.setImageResource(R.drawable.ic_assignment_blue_24dp); //img icon

        holder.tv_titleAtt.setText("Excuse Request");  // title
        String time[] = logExcusesModel.getDate().split("T");
        holder.tv_dateAtt.setText("Date : "  + time[0]);
        holder.tv_timeAtt.setText("Status : " + logExcusesModel.getStatus());
        holder.tv_locationAtt.setText(" "); //"Message : " + logExcusesModel.getExMessage()
